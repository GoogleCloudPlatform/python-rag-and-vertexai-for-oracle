...
# MODIFIED: Import get_table_schema, query_database, and list_all_tables
...
def setup_langchain_agent(verbose_langchain_executor: bool = False):
    ...
    # MODIFIED: Include all necessary tools
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system",
             ...
             "**Important Tool Usage Order & Guidance:**\n"
             "1. **Discover Tables (`list_all_tables`):** If the user asks 'What tables do you have?' or 'List tables', or if you are completely unsure which table contains the requested information (e.g., first query after a new session and table name is not specified), use the `list_all_tables()` tool. This tool will provide a list of available tables along with their descriptions.\n"
             "2. **Understand Schema (`get_table_schema`):** Once you know the relevant table, use the `get_table_schema(table_name)` tool to understand its structure (column names and types). This is crucial for formulating correct queries, especially for column names, data types, and required conditions. For example, if the user asks about 'bike trips', infer the table is 'CITI_BIKE' and then get its schema to confirm column names like 'STARTED_AT'.\n" # MODIFIED: Emphasize schema importance for column names
             "3. **Query Data (`query_database`):** Use the `query_database` tool for ALL data operations: retrieving records, counting records, and performing aggregations (like finding most common items, or earliest/latest dates/timestamps). Do NOT execute SQL directly. Always pass components to the `query_database` tool's parameters.\n"
             "\n"
             ...
             "- `select_columns`: Specify columns to retrieve (e.g., 'MAKE, MODEL') or aggregate functions (e.g., 'COUNT(*) AS TotalRecords', 'MIN(QUOTE_DATE) AS EarliestDate', 'MAX(STARTED_AT) AS LatestTripStart', 'MAKE, COUNT(*) AS MakeCount'). Default is '*'.\n" # MODIFIED: Updated examples with actual date/timestamp column names
             "- `conditions`: An optional SQL WHERE clause. When comparing strings (VARCHAR2), use single quotes (e.g., \"QUOTE_SYMBOL = 'GOOG'\"). For partial matches, use LIKE (e.g., \"MODEL LIKE 'Tesla%'\"). For dates/timestamps, use `TO_DATE('YYYY-MM-DD', 'YYYY-MM-DD')` or `TO_TIMESTAMP('YYYY-MM-DD HH24:MI:SS', 'YYYY-MM-DD HH24:MI:SS')` as needed. \n" # MODIFIED: Added specific guidance for string and date/timestamp conditions
